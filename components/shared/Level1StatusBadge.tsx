...
export default function Level1StatusBadge() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      <div 
        className={cn(
          "transition-all duration-300 ease-in-out overflow-hidden shadow-2xl border bg-[#020617]/95 backdrop-blur-xl border-emerald-500/20",
      ...
      >
        {!isOpen ? (
          <div className="flex h-full items-center justify-center gap-1.5 px-3">
            <span className="text-[10px] font-black text-emerald-500 uppercase tracking-tighter">LV1</span>
            {status.ready ? (
              <ShieldCheck className="h-4 w-4 text-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
            ) : (
          ...
          <div className="p-5">
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-white/5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-white italic">Governance Sync</span>
              </div>
              <button 
                onClick={(e) => {
                  e.stopPropagation()
            ...
            <div className="space-y-3">
              <StatusItem label="Identity Linked" active={status.installed} />
              <StatusItem label="Consensus Synced" active={status.testnet} />
              <StatusItem label="Ledger Decoded" active={status.balanceLoaded} />
              <StatusItem label="Voting Power Active" active={status.ready} />
            </div>

            <div className="mt-4 pt-3 border-t border-white/5">
               <div className="flex items-center justify-between mb-2">
                  <span className="text-[8px] font-black text-muted-foreground/40 uppercase tracking-widest">Protocol Compliance</span>
                  <span className="text-[10px] font-black text-emerald-500 italic">BELT L1 ✓</span>
               </div>
               <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-emerald-600 shadow-[0_0_8px_rgba(16,185,129,0.6)] transition-all duration-700" 
